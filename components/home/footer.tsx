...
async function getBrands() {
    const res = await fetch(
        "https://smpl.supermartng.com/brand?page=1&limit=500&access_id=ylemrZr7QzQKOVujBBSr&session_id=3djwLXseHgZ7BdWcyharj9QwaBTwWsPsLVxqk4OysAuKtI9GCe"
    );
...
export async function HomeFooter() {
    ...
    return (
        <div className="-mx-12 bg-white mt-8 pt-4 px-4">
                ...
                <div className="flex flex-wrap justify-center gap-8">
                    {homePartners.map((homePartner: any, i: number) => (
                        <div key={i} className="relative w-28 h-28">
                            <Image
                                src={`http://s3-static-play.supermartng.com/supermart-staging${homePartner.image}`}
                ...
                <div className="m-4">
                    ...
                    <div className="">
                        {brands.map((brand: any, k: number) => (
                            <Link key={k} className="text-xs font-semibold" href={`/brand/products/${brand.name}`}>
